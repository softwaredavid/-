...
@synthesize m_bSwiftFlag;
SimpleDemoViewController *g_pController = NULL;
HikSwift *test = [[HikSwift alloc] init];
...
// get ezvizlife accesstoken by HC
//    EzvizLifeViewController *ezvizlife = [[EzvizLifeViewController alloc]initWithNibName:@"EzvizLifeViewController" bundle:[NSBundle mainBundle]];
//    [self.navigationController pushViewController:ezvizlife animated:YES];
/*    ezvizlife.returnTextBlock = ^(NSString *retUrl){
        NSLog(@"retUrl:%@",retUrl);
        if([retUrl rangeOfString:@"success"].location != NSNotFound){
            
            NSString *accessToken = [retUrl substringWithRange:NSMakeRange(58, 64)];
...
{    
    ...
    {
        NSLog(@"NET_DVR_CapturePictureBlock succ[%s]", szFileName);
    }
    else
    {
        NSLog(@"NET_DVR_CapturePictureBlock failed[%d]", NET_DVR_GetLastError());
    }
    
    return;
    
//    if (m_lRealPlayID >= 0)  //预览抓图
//    {
//        if(NET_DVR_CapturePictureBlock(m_lRealPlayID, szFileName, 0))
//        {
//            NSLog(@"NET_DVR_CapturePictureBlock succ[%s]", szFileName);
...
//        }
//    }
//    else if(m_lPlaybackID >= 0)  //回放抓图
//    {
//        if(!NET_DVR_PlayBackCaptureFile(m_lPlaybackID, szFileName))
//        {
//            NSLog(@"NET_DVR_PlayBackCaptureFile failed[%d]", NET_DVR_GetLastError());
//        }
//        else
//        {
//            NSLog(@"NET_DVR_PlayBackCaptureFile succ[%s]", szFileName);
//        }
//    }
//    else
//    {
//        NSLog(@"Please start preview or playback first!");
//        return;
//    }
//    return;
}
...
-(IBAction) playerBtnClicked:(id)sender
{
    ...
    else
    {
        if(!m_bPreview)
        {
            if(m_bSwiftFlag)
            {
                m_lRealPlayID = [test Test_RealPlayWithHPlayWnd:m_playView];
            }
            else
            {
        ...
        else
        {
            if(m_bSwiftFlag)
            {
                [test Test_StopRealPlayWithLRealHandle:m_lRealPlayID];
            }
            else
            {
                stopPreview(0);
...
-(IBAction) playbackBtnClicked:(id)sender
{
	...
	NSLog(@"playbackBtnClicked");
       if (m_lPlaybackID == -1)
       {
           ...
           struStartTime.dwYear = 2017;
           struStartTime.dwMonth = 7;
           struStartTime.dwDay = 16;
           
           struEndTime.dwYear = 2017;
           struEndTime.dwMonth = 9;
           struEndTime.dwDay = 6;
    ...
    //获取播放库句柄
           int iPlaybackIndex = NET_DVR_GetPlayBackPlayerIndex(m_lPlaybackID);
           if(iPlaybackIndex != -1)
           {
               NSLog(@"NET_DVR_GetPlayBackPlayerIndex Succ, index = %d",  iPlaybackIndex);
           }
           else
           {
               NSLog(@"NET_DVR_GetPlayBackPlayerIndex fail:%d",  NET_DVR_GetLastError());
               [self stopPlayback];
               return;
           }
    //回放控制
           if (NET_DVR_PlayBackControl_V40(m_lPlaybackID, NET_DVR_PLAYPAUSE, NULL, 0, NULL, NULL))
           {
               NSLog(@"NET_DVR_PLAYPAUSE succ");
           }
           if (NET_DVR_PlayBackControl_V40(m_lPlaybackID, NET_DVR_PLAYRESTART, NULL, 0, NULL, NULL))
           {
               NSLog(@"NET_DVR_PLAYRESTART succ");
           }
           if (NET_DVR_PlayBackControl_V40(m_lPlaybackID, NET_DVR_PLAYFAST, NULL, 0, NULL, NULL))
           {
               NSLog(@"NET_DVR_PLAYFAST succ");
           }
           if (NET_DVR_PlayBackControl_V40(m_lPlaybackID, NET_DVR_PLAYSLOW, NULL, 0, NULL, NULL))
           {
               NSLog(@"NET_DVR_PLAYSLOW succ");
           }
           if (NET_DVR_PlayBackControl_V40(m_lPlaybackID, NET_DVR_PLAYSTARTAUDIO, NULL, 0, NULL, NULL))
           {
               NSLog(@"NET_DVR_PLAYSTARTAUDIO succ");
           }
           
           if (!NET_DVR_PlayBackControl(m_lPlaybackID, NET_DVR_PLAYAUDIOVOLUME, (0xffff)/2, NULL))
           {
               NSLog(@"NET_DVR_PLAYAUDIOVOLUME fail:%d", NET_DVR_GetLastError());
           }
           else{
               NSLog(@"NET_DVR_PLAYAUDIOVOLUME succ");
           }
           if (!NET_DVR_PlayBackControl_V40(m_lPlaybackID, NET_DVR_PLAYSTOPAUDIO, NULL, 0, NULL, NULL))
           {
               NSLog(@"NET_DVR_PLAYSTOPAUDIO fail:%d", NET_DVR_GetLastError());
           }
           else{
               NSLog(@"NET_DVR_PLAYSTOPAUDIO succ");
           }
...
-(IBAction) playbackBtnClicked:(id)sender
{
    ...
    if (m_lPlaybackID == -1)
    {
        ...
        m_lPlaybackID = NET_DVR_PlayBackByName(m_lUserID, "ch0001_02000000012000000", m_playView);
        if (m_lPlaybackID == -1)
        {
...
//        if([self loginEZVIZDevice])
    ...
    else 
    {
        if(m_bSwiftFlag)
        {
            [test Test_LogoutWithLUserID:m_lUserID];
            m_lUserID = -1;
        }
        else
        {
...
- (void)viewDidLoad 
{
    ...
    m_deviceIpField.text = @"10.17.133.46";
    m_devicePortField.text = @"8000";
    m_uerNameField.text = @"admin";
    m_passwordField.text = @"hik12345";
